...
impl<'src> Lexer<'src> {
    ...
    fn skip_whitespace_and_comments(&mut self) -> Result<(), LexError> {
        loop {
            ...
            if self.peek() == Some('/') && self.peek2() == Some('*') {
                ...
                loop {
                    match self.peek() {
                        None => return Err(LexError::UnterminatedBlockComment { line: comment_line }),
    ...
    fn lex_number(&mut self, start: usize) -> Result<Spanned<Token>, LexError> {
            ...
            && self.peek2().map(|c| c.is_ascii_digit()).unwrap_or(false)
        {
            ...
            let val: f64 = text.parse().map_err(|_| LexError::IntegerOverflow { line })?;
            return Ok((Token::FloatLit(val), (start, self.pos)));
        }

        // Plain integer
        let text = &self.src[start..self.pos];
        let val: u64 = text.parse().map_err(|_| LexError::IntegerOverflow { line })?;
    ...
    fn lex_punctuation(&mut self, start: usize) -> Result<Spanned<Token>, LexError> {
        macro_rules! tok {
            ($t:expr) => {{ self.advance(); Ok(($t, (start, self.pos))) }};
        }
        macro_rules! tok2 {
            ($t:expr) => {{ self.advance(); self.advance(); Ok(($t, (start, self.pos))) }};
        }
        macro_rules! tok2_or {
            ($two_ch:expr, $two_tok:expr, $one_tok:expr) => {
                if self.peek2() == Some($two_ch) { tok2!($two_tok) } else { tok!($one_tok) }
            };
        }
        ...
        match c {
            ...
            '+' => {
                if self.peek2() == Some('+')  { tok2!(Token::PlusPlus) }
                else if self.peek2() == Some('=') { tok2!(Token::PlusEq) }
                else { tok!(Token::Plus) }
            }
            '-' => {
                if self.peek2() == Some('-')  { tok2!(Token::MinusMinus) }
                else if self.peek2() == Some('=') { tok2!(Token::MinusEq) }
                else if self.peek2() == Some('>') { tok2!(Token::Arrow) }
                else { tok!(Token::Minus) }
            }
            '*' => tok2_eq_or!(Token::StarEq, Token::Star),
            '/' => tok2_eq_or!(Token::SlashEq, Token::Slash),
            '&' => {
                if self.peek2() == Some('&')  { tok2!(Token::AmpAmp) }
                else if self.peek2() == Some('=') { tok2!(Token::AmpEq) }
                else { tok!(Token::Amp) }
            }
            '|' => {
                if self.peek2() == Some('|')  { tok2!(Token::PipePipe) }
                else if self.peek2() == Some('=') { tok2!(Token::PipeEq) }
                else { tok!(Token::Pipe) }
            }
            '<' => {
                if self.peek2() == Some('<') {
                    self.advance(); self.advance();
                    if self.peek() == Some('=') { self.advance(); Ok((Token::ShlEq, (start, self.pos))) }
                    else { Ok((Token::Shl, (start, self.pos))) }
                } else if self.peek2() == Some('=') { tok2!(Token::LtEq) }
                else { tok!(Token::Lt) }
            }
            '>' => {
                if self.peek2() == Some('>') {
                    self.advance(); self.advance();
                    if self.peek() == Some('=') { self.advance(); Ok((Token::ShrEq, (start, self.pos))) }
                    else { Ok((Token::Shr, (start, self.pos))) }
                } else if self.peek2() == Some('=') { tok2!(Token::GtEq) }
                else { tok!(Token::Gt) }
            }
            _ => {
...
fn keyword_or_ident(name: String) -> Token {
    match name.as_str() {
        // types
        "U0"  => Token::U0,
        "I8"  => Token::I8,  "U8"  => Token::U8,
        "I16" => Token::I16, "U16" => Token::U16,
        "I32" => Token::I32, "U32" => Token::U32,
        "I64" => Token::I64, "U64" => Token::U64,
