use holyc_frontend::{Lexer, Parser, token::Token};
...
fn lex_integer_literals() {
    ...
    let toks: Vec<Token> = lex.tokenize().unwrap().into_iter().map(|(t,_)|t).collect();
...
fn parse_empty_function() {
    ...
    match &m.items[0].kind {
        TopLevelKind::FuncDef { name, params, body, .. } => {
            assert_eq!(name, "Main");
...
fn parse_function_with_params() {
    ...
    use holyc_frontend::ast::{TopLevelKind, StmtKind};
    match &m.items[0].kind {
        TopLevelKind::FuncDef { name, params, body, .. } => {
            assert_eq!(name, "Add");
...
fn parse_var_decl_with_init() {
    ...
    use holyc_frontend::ast::{TopLevelKind, StmtKind, ExprKind};
    match &m.items[0].kind {
...
fn parse_if_else() {
    ...
    match &m.items[0].kind {
        TopLevelKind::FuncDef { body, .. } => {
            assert!(matches!(body[0].kind, StmtKind::If { else_body: Some(_), .. }));
        }
        other => panic!("{other:?}"),
    }
}
