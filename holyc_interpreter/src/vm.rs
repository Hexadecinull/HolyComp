...
use holyc_frontend::ast::{
    AssignOp, BinOp, ExprKind, Module, StmtKind, TopLevelKind, UnaryOp,
};
...
impl Interpreter {
    ...
    pub fn exec_module(&mut self, module: &Module) -> Result<(), RuntimeError> {
        // Pass 1: hoist all function definitions so call order doesn't matter.
        for item in &module.items {
            if let TopLevelKind::FuncDef { name, params, body, .. } = &item.kind {
                self.funcs.insert(
                    name.clone(),
    ...
    pub fn call_func(&mut self, name: &str, args: &[Value]) -> Result<Value, RuntimeError> {
        ...
        match name {
            ...
            "StrLen" | "strlen" => {
                ...
                match args.first() {
                    ...
                    _ => return Err(RuntimeError::Custom("StrLen: expected string or pointer".into())),
                }
            }
            _ => {}
        }
        ...
        let args = if STR_BUILTINS.contains(&name) {
            resolved_args = args.iter().map(|v| match v {
                Value::Ptr(p) if *p != 0 => {
                    self.heap.read_cstr(*p)
                        .map(Value::Str)
        ...
        let callable = self.funcs.get(name)
    ...
    fn invoke(&mut self, callable: Callable, _name: &str, args: &[Value]) -> Result<Value, RuntimeError> {
        match callable {
        ...
        match &stmt.kind {
            ...
            StmtKind::Switch { expr, cases } => {
                ...
                'switch: for case in cases {
                    let should_run = matched || match &case.value {
                        None => true,
                        Some(e) => {
    ...
    pub fn eval_expr(&mut self, expr: &holyc_frontend::ast::Expr) -> Result<Value, RuntimeError> {
        match &expr.kind {
            ...
            ExprKind::Ternary { cond, then, else_ } => {
                ...
                if v.is_truthy() { self.eval_expr(then) } else { self.eval_expr(else_) }
            }
            ...
            ExprKind::SizeOfType(ty) => Ok(Value::UInt(ty.size_of().unwrap_or(0))),
            ExprKind::SizeOfExpr(e)  => {
                let _ = self.eval_expr(e)?;
                Ok(Value::UInt(8)) // placeholder – proper impl needs type inference
            }

            // Pointer / subscript / member: deferred to bytecode VM / LLVM backend.
            ExprKind::Index { .. } | ExprKind::Member { .. } => {
                Err(RuntimeError::Custom(
                    "pointer / member access requires the bytecode VM or LLVM backend".into()
                ))
            }
        }
    }
    ...
    fn eval_binary(
        ...
        if op == BinOp::LogAnd {
            ...
            return if !l.is_truthy() { Ok(Value::Bool(false)) }
                   else { Ok(Value::Bool(self.eval_expr(rhs)?.is_truthy())) };
        }
        if op == BinOp::LogOr {
            let l = self.eval_expr(lhs)?;
        ...
        if matches!((&l, &r), (Value::Float(_), _) | (_, Value::Float(_))) {
            let lf = l.as_float().ok_or_else(|| type_error("float", l.type_name()))?;
            let rf = r.as_float().ok_or_else(|| type_error("float", r.type_name()))?;
            return Ok(match op {
                BinOp::Add => Value::Float(lf + rf),
                BinOp::Sub => Value::Float(lf - rf),
                BinOp::Mul => Value::Float(lf * rf),
                BinOp::Div => {
                    if rf == 0.0 { return Err(RuntimeError::Custom("division by zero".into())); }
                    Value::Float(lf / rf)
                }
                ...
                _ => return Err(RuntimeError::Custom(
                    format!("operator {op:?} not defined for floats")
                )),
            });
        }

        // Integer arithmetic
        let li = l.as_int().ok_or_else(|| type_error("integer", l.type_name()))?;
        let ri = r.as_int().ok_or_else(|| type_error("integer", r.type_name()))?;
        Ok(match op {
            BinOp::Add    => Value::Int(li.wrapping_add(ri)),
            BinOp::Sub    => Value::Int(li.wrapping_sub(ri)),
            BinOp::Mul    => Value::Int(li.wrapping_mul(ri)),
            BinOp::Div => {
                if ri == 0 { return Err(RuntimeError::Custom("division by zero".into())); }
                Value::Int(li / ri)
            }
            BinOp::Rem => {
                if ri == 0 { return Err(RuntimeError::Custom("division by zero".into())); }
    ...
    fn eval_assign(
        ...
        if let ExprKind::Unary { op: UnaryOp::Deref, operand } = &lhs.kind {
...
fn apply_int_binop(op: BinOp, l: Value, r: Value) -> Result<Value, RuntimeError> {
    ...
    Ok(match op {
        ...
        BinOp::Shl    => Value::Int(li.wrapping_shl(ri as u32)),
        BinOp::Shr    => Value::Int(li.wrapping_shr(ri as u32)),
        _ => return Err(RuntimeError::Custom(format!("{op:?} not valid for compound assignment"))),
    })
}
