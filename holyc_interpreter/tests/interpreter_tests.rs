...
fn neg() {
    assert_eq!(
        call("I64 F(I64 x){return -x;}", "F", &[Value::Int(42)]),
        Value::Int(-42)
    );
}
#[test]
fn shl() {
    assert_eq!(
        call("I64 F(I64 x){return x<<3;}", "F", &[Value::Int(1)]),
        Value::Int(8)
    );
}
#[test]
fn shr() {
    assert_eq!(
        call("I64 F(I64 x){return x>>2;}", "F", &[Value::Int(64)]),
        Value::Int(16)
    );
}
#[test]
fn band() {
    assert_eq!(
        call(
            "I64 F(I64 a,I64 b){return a&b;}",
            "F",
            &[Value::Int(0b1100), Value::Int(0b1010)]
        ),
        Value::Int(0b1000)
    );
}
...
fn and_short() {
    assert_eq!(
        call("Bool F(){return FALSE&&(1/0==0);}", "F", &[]),
        Value::Bool(false)
    );
}
#[test]
fn or_short() {
    assert_eq!(
        call("Bool F(){return TRUE||(1/0==0);}", "F", &[]),
        Value::Bool(true)
    );
}
...
fn var_init() {
    assert_eq!(
        call("I64 F(){I64 x=99;return x;}", "F", &[]),
        Value::Int(99)
    );
}
#[test]
fn add_assign() {
    assert_eq!(
        call("I64 F(){I64 x=10;x+=5;return x;}", "F", &[]),
        Value::Int(15)
    );
}
#[test]
fn sub_assign() {
    assert_eq!(
        call("I64 F(){I64 x=10;x-=3;return x;}", "F", &[]),
        Value::Int(7)
    );
}
#[test]
fn mul_assign() {
    assert_eq!(
        call("I64 F(){I64 x=4;x*=3;return x;}", "F", &[]),
        Value::Int(12)
    );
}
#[test]
fn pre_inc() {
    assert_eq!(
        call("I64 F(){I64 x=5;++x;return x;}", "F", &[]),
        Value::Int(6)
    );
}
#[test]
fn post_inc_old() {
    assert_eq!(
        call("I64 F(){I64 x=5;I64 y=x++;return y;}", "F", &[]),
        Value::Int(5)
    );
}
#[test]
...
fn while_sum() {
    let v = call(
        "I64 F(I64 n){I64 s=0;I64 i=1;while(i<=n){s+=i;i++;}return s;}",
        "F",
        &[Value::Int(10)],
    );
...
fn for_sum() {
    let v = call(
        "I64 F(I64 n){I64 s=0;for(I64 i=1;i<=n;i++){s+=i;}return s;}",
        "F",
        &[Value::Int(100)],
    );
...
fn break_loop() {
    let v = call(
        "I64 F(){I64 i=0;while(TRUE){if(i==7)break;i++;}return i;}",
        "F",
        &[],
    );
...
fn continue_loop() {
    let v = call(
        "I64 F(){I64 s=0;for(I64 i=0;i<10;i++){if(i%2!=0)continue;s+=i;}return s;}",
        "F",
        &[],
    );
...
fn ternary_true() {
    assert_eq!(
        call("I64 F(Bool b){return b?1:2;}", "F", &[Value::Bool(true)]),
        Value::Int(1)
    );
}
#[test]
fn ternary_false() {
    assert_eq!(
        call("I64 F(Bool b){return b?1:2;}", "F", &[Value::Bool(false)]),
        Value::Int(2)
    );
}
...
fn sizeof_i64() {
    assert_eq!(
        call("I64 F(){return sizeof(I64);}", "F", &[]),
        Value::UInt(8)
    );
}
#[test]
...
#[test]
fn builtin_abs() {
    assert_eq!(call("I64 F(){return Abs(-42);}", "F", &[]), Value::Int(42));
}
#[test]
fn builtin_sqrt() {
    assert_eq!(
        call("F64 F(){return Sqrt(9.0);}", "F", &[]),
        Value::Float(3.0)
    );
}
#[test]
fn builtin_strlen() {
    assert_eq!(
        call("I64 F(){return StrLen(\"hello\");}", "F", &[]),
        Value::Int(5)
    );
}
...
#[test]
fn div_zero_errors() {
    assert!(call_err("I64 F(){return 1/0;}", "F", &[]));
}
#[test]
fn undef_var_errors() {
    assert!(call_err("I64 F(){return x;}", "F", &[]));
}
#[test]
fn undef_func_errors() {
...
fn global_var_read() {
    assert_eq!(
        call("I64 g = 7; I64 F() { return g; }", "F", &[]),
        Value::Int(7)
    );
}

#[test]
fn global_var_mutate() {
    assert_eq!(
        call(
            "I64 g = 0; U0 Inc() { g += 1; } I64 F() { Inc(); Inc(); return g; }",
            "F",
            &[]
        ),
        Value::Int(2)
...
fn do_while_runs_once() {
    assert_eq!(
        call(
            "I64 F() { I64 x = 0; do { x = 1; } while (FALSE); return x; }",
            "F",
            &[]
        ),
        Value::Int(1)
...
fn do_while_sum() {
    assert_eq!(
        call(
            "I64 F() { I64 s = 0; I64 i = 1; do { s += i; i++; } while (i <= 5); return s; }",
            "F",
            &[]
        ),
        Value::Int(15)
...
// ── Heap / pointer round-trip ─────────────────────────────────────────────────
#[test]
fn heap_alloc_write_read() {
    assert_eq!(
        call(
            "I64 F() { I64* p = MAlloc(8); *p = 99; I64 v = *p; Free(p); return v; }",
            "F",
            &[]
        ),
        Value::Int(99)
...
#[test]
fn heap_memset_zeroes() {
    assert_eq!(
        call(
            "I64 F() { I64* p = MAlloc(8); *p = 42; MemSet(p, 0, 8); I64 v = *p; Free(p); return v; }",
            "F",
            &[]
        ),
        Value::Int(0)
...
// ── String builtins ───────────────────────────────────────────────────────────
#[test]
fn builtin_strcmp_equal() {
    assert_eq!(
        call("I64 F() { return StrCmp(\"abc\", \"abc\"); }", "F", &[]),
        Value::Int(0)
    );
}
...
fn builtin_strcat() {
    assert_eq!(
        call(
            "I64 F() { return StrLen(StrCat(\"foo\", \"bar\")); }",
            "F",
            &[]
        ),
        Value::Int(6)
...
#[test]
fn builtin_strstr_found() {
    assert_eq!(
        call("I64 F() { return StrStr(\"foobar\", \"bar\"); }", "F", &[]),
        Value::Int(3)
    );
}

#[test]
fn builtin_strstr_miss() {
    assert_eq!(
        call("I64 F() { return StrStr(\"foobar\", \"xyz\"); }", "F", &[]),
        Value::Int(-1)
    );
}

#[test]
fn builtin_str_to_i64() {
    assert_eq!(
        call("I64 F() { return StrToI64(\"12345\"); }", "F", &[]),
        Value::Int(12345)
    );
}
...
//
// The test runs in a thread with a generous stack so the interpreter's own
// MAX_CALL_DEPTH guard fires cleanly before the OS stack is exhausted.
// Without a dedicated thread, 512 levels of mutually-recursive Rust calls
// (~3 000 Rust frames total) can blow the 8 MiB default test-thread stack.
#[test]
fn stack_overflow_errors() {
    let result = std::thread::Builder::new()
        .stack_size(64 * 1024 * 1024) // 64 MiB — well above what 512 interpreter frames need
        .spawn(|| call_err("I64 F(I64 n) { return F(n + 1); }", "F", &[Value::Int(0)]))
        .expect("thread spawn failed")
        .join()
        .expect("thread panicked");
    assert!(
        result,
        "expected a stack-overflow error from the interpreter depth guard"
    );
}
...
fn cast_i64() {
    assert_eq!(
        call("I64 F(F64 x) { return (I64)x; }", "F", &[Value::Float(3.9)]),
        Value::Float(3.9) // interpreter pass-through; LLVM backend truncates
    );
}
...
fn sizeof_expr_bool() {
    assert_eq!(
        call("I64 F() { Bool b = TRUE; return sizeof(b); }", "F", &[]),
        Value::UInt(1)
    );
}
...
fn sizeof_f64() {
    assert_eq!(
        call("I64 F() { return sizeof(F64); }", "F", &[]),
        Value::UInt(8)
    );
}
